...
impl SmartResolver {
    pub fn new(router: Arc<DomainMatcher>, regionconf: &DnsProxy) -> Result<SmartResolver, Error> {
        let rresolvers: Vec<(Bytes, DnsClient)> = regionconf
            .forward
            .iter()
            .map(|(r, s)| {
        ...
        let name = {
            ...
            if queries.len() != 1 {
                return Err(format_err!(
                    "more than 1 queries in a message: {:?}",
                    message
                ));
            }
            let q = &queries[0];
